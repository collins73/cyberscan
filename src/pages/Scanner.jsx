...
Also provide an overall security score from 0-100 (100 being most secure).`,
      ...
      if (enrichedVulnerabilities && enrichedVulnerabilities.length > 0) {
        ...
        // Auto-create alerts for critical/high vulns
        const critical = enrichedVulnerabilities.filter(v => v.severity === 'critical');
        const high = enrichedVulnerabilities.filter(v => v.severity === 'high');
        if (critical.length > 0) {
          await base44.entities.SecurityAlert.create({
            alert_type: 'critical_vulnerability',
            severity: 'critical',
            title: `${critical.length} Critical Vulnerability${critical.length > 1 ? 's' : ''} Detected`,
            description: `Critical issues found in ${fileName}: ${critical.map(v => v.title).join(', ')}`,
            app_name: fileName,
            scan_id: savedScan.id,
            status: 'active'
          });
          ...
          await base44.entities.SecurityAlert.create({
            ...
            severity: 'high',
            title: `${high.length} High Severity Issue${high.length > 1 ? 's' : ''} in ${fileName}`,
            description: `High severity vulnerabilities: ${high.map(v => v.title).join(', ')}`,
            app_name: fileName,
            scan_id: savedScan.id,
            status: 'active'
          });
...
Be specific and cite actual CVE numbers, CISA advisories, or NIST NVD data when available.`,
          ...
          response_json_schema: {
            ...
            properties: {
              ...
              attack_patterns: { type: "array", items: { type: "string" } },
              threat_landscape: {
                type: "object",
                properties: {
        ...
        return { ...vuln, threat_intelligence: threatIntel };
      } catch (error) {
  ...
  const handleViewScan = (scan) => { setCurrentScan(scan); setView('results'); };
  const handleBackToInput = () => { setView('input'); setCurrentScan(null); };
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          {isScanning ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h2 className="text-2xl font-bold text-white mt-8 mb-2">Scanning Code...</h2>
