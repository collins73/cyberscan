...
import { X, GitPullRequest, Loader2, CheckCircle, AlertTriangle, ExternalLink, Zap } from 'lucide-react';
...
export default function AutoFixModal({ scanData, onClose }) {
  ...
  const validateAndSubmit = () => {
    ...
    if (!repoFullName.trim() || !repoFullName.includes('/')) {
      setRepoError('Must be in owner/repo format (e.g. collins73/cyberscan)');
      valid = false;
    } else {
      setRepoError('');
    }
    ...
    if (!valid || !originalCode.trim() || selectedVulns.length === 0) return;
    handleSubmit();
  };
  ...
  const isSubmitDisabled = !repoFullName.trim() || !repoFullName.includes('/') || !filePath.trim() || !originalCode.trim() || selectedVulns.length === 0;
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-green-500/10 rounded-lg"><GitPullRequest className="w-5 h-5 text-green-400" /></div>
            <div>
        ...
        <div className="p-6 space-y-6">
          {stage === 'configure' && (
            <>
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-slate-400 text-xs mb-1 block">Repo <span className="text-red-400">*</span> <span className="text-slate-500">(owner/repo-name)</span></label>
                  <input value={repoFullName} onChange={e => { setRepoFullName(e.target.value); setRepoError(''); }}
                    placeholder="e.g. collins73/cyberscan"
                    className={`w-full bg-slate-950 border text-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none ${repoError ? 'border-red-500/70' : 'border-slate-700 focus:border-green-500/60'}`} />
                  {repoError && <p className="text-red-400 text-xs mt-1">{repoError}</p>}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-slate-400 text-xs mb-1 block">File path in repo <span className="text-red-400">*</span></label>
                    <input value={filePath} onChange={e => { setFilePath(e.target.value); setFilePathError(''); }}
                      placeholder="e.g. src/components/App.jsx"
                      className={`w-full bg-slate-950 border text-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none ${filePathError ? 'border-red-500/70' : 'border-slate-700 focus:border-green-500/60'}`} />
                    {filePathError ? <p className="text-red-400 text-xs mt-1">{filePathError}</p> : <p className="text-slate-500 text-xs mt-1">Full path within the repo</p>}
                  </div>
                  <div>
              ...
              <div>
                <label className="text-slate-400 text-xs mb-1 block">Original file content <span className="text-red-400">*</span></label>
                <textarea value={originalCode} onChange={e => setOriginalCode(e.target.value)} rows={6}
              ...
              <div>
                <h3 className="text-slate-300 text-sm font-semibold uppercase tracking-wide mb-2">Select Vulnerabilities to Fix ({selectedVulns.length} selected)</h3>
                <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                  {fixableVulns.map((v, i) => (
                    <label key={i} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${selectedVulnIds.includes(i) ? 'border-green-500/40 bg-green-500/5' : 'border-slate-800 bg-slate-950/40 hover:border-slate-700'}`}>
                      <input type="checkbox" checked={selectedVulnIds.includes(i)} onChange={() => toggleVuln(i)} className="mt-0.5 accent-green-500" />
              ...
              <Button onClick={validateAndSubmit} disabled={isSubmitDisabled}
                ...
                <Zap className="w-4 h-4 mr-2" />Generate Fix & Open Pull Request
              </Button>
            </>
          )}
          ...
          {stage === 'error' && (
            <div className="text-center py-10 space-y-4">
              ...
              <Button onClick={() => setStage('configure')} variant="outline" className="border-slate-700 text-slate-300">Back to Configure</Button>
            </div>
          )}
            ...
            <div className="space-y-5">
              ...
              <a href={result.pr_url} target="_blank" rel="noopener noreferrer"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-semibold">View Pull Request #{result.pr_number}</p>
                    <p className="text-slate-400 text-xs truncate max-w-xs">{result.pr_url}</p>
                  </div>
                </div>
                  ...
                  <ul className="space-y-1">
                    {result.changes.map((c, i) => (
                      <li key={i} className="flex items-start gap-2 text-slate-400 text-sm">
                        <CheckCircle className="w-4 h-4 text-green-400 mt-0.5 shrink-0" />{c}
                      </li>
                    ))}
                  </ul>
                </div>
              )}
              <Button onClick={onClose} className="w-full bg-slate-800 hover:bg-slate-700 text-white">Close</Button>
            </div>
          )}
        </div>
      </motion.div>
